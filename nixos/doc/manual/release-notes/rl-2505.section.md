...
- `services.pdns-recursor` [PowerDNS Recursor New Style (YAML) Settings](https://docs.powerdns.com/recursor/yamlsettings.html)
  - pdns-recursor.settings has a similar format to the official YAML configuration.
  - The old options have been taken over, but in the YAML structure.
  - The configuration is entirely in pdns-recursor.settings and can be extended (YAML freeformType).
  - The parameters defined in the settings should be reviewed (referring to the official pdns-recursor documentation)
